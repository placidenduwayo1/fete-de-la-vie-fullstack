...
import { EntityArrayResponseType, EntityResponseType } from './logo-teaser.service';
import { EMPTY, Observable, firstValueFrom, map, mergeMap, of } from 'rxjs';
...
export class FdlvUsersService {
  ...
  getUserLogin = async (fusId: number): Promise<string> => {
    ...
    const fdlvUser = await firstValueFrom(this.getUser(fusId));
    if (fdlvUser.login) {
