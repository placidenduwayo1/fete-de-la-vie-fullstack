...
export SPRING_PROFILES_ACTIVE=gitlab-ci
...
# Get the version of the project from the pom.xml
tag=$(cat pom.xml | grep -m 1 '<version>' | grep -Eo "[[:digit:]]+\.[[:digit:]]+\.[[:digit:]]+").$CI_COMMIT_SHORT_SHA.$POSTFIX

# Create a tag in Gitlab
echo "Creating tag $tag in Gitlab from the version found in pom.xml..."
git tag "$tag"
