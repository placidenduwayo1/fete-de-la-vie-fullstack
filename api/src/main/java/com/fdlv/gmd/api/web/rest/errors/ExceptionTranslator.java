...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
	...
	public ResponseEntity<Problem> process(@Nullable ResponseEntity<Problem> entity, NativeWebRequest request) {
		...
		final String requestUri = nativeRequest != null ? nativeRequest.getRequestURI() : StringUtils.EMPTY;
	...
	public ResponseEntity<Problem> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		final List<FieldErrorVM> fieldErrors = result.getAllErrors().stream()
				.map(f -> new FieldErrorVM(f.getObjectName().replaceFirst("DTO$", ""),
						f.getDefaultMessage().split(":")[0].strip(),
						StringUtils.isNotBlank(f.getDefaultMessage()) ? f.getDefaultMessage().split(":")[1].strip()
								: f.getCode()))
				.collect(Collectors.toList());

		final Problem problem = Problem.builder().withType(ErrorConstants.CONSTRAINT_VIOLATION_TYPE)
				.withTitle("Method argument not valid").withStatus(this.defaultConstraintViolationStatus())
	...
	public ResponseEntity<Problem> handleEmailAlreadyUsedException(
			com.fdlv.gmd.api.service.EmailAlreadyUsedException ex, NativeWebRequest request) {
		final EmailAlreadyUsedException problem = new EmailAlreadyUsedException();
		return this.create(problem, request, HeaderUtil.createFailureAlert(this.applicationName, false,
	...
	public ResponseEntity<Problem> handleUsernameAlreadyUsedException(
			com.fdlv.gmd.api.service.UsernameAlreadyUsedException ex, NativeWebRequest request) {
		final LoginAlreadyUsedException problem = new LoginAlreadyUsedException();
		return this.create(problem, request, HeaderUtil.createFailureAlert(this.applicationName, false,
				problem.getEntityName(), problem.getErrorKey(), problem.getMessage()));
	}

	@ExceptionHandler
	public ResponseEntity<Problem> handleInvalidPasswordException(com.fdlv.gmd.api.service.InvalidPasswordException ex,
			NativeWebRequest request) {
		return this.create(new InvalidPasswordException(), request);
	}

	@ExceptionHandler
	public ResponseEntity<Problem> handleBadRequestAlertException(BadRequestAlertException ex,
			NativeWebRequest request) {
		return this.create(ex, request, HeaderUtil.createFailureAlert(this.applicationName, false, ex.getEntityName(),
				ex.getErrorKey(), ex.getMessage()));
	}

	@ExceptionHandler
	public ResponseEntity<Problem> handleConcurrencyFailure(ConcurrencyFailureException ex, NativeWebRequest request) {
		final Problem problem = Problem.builder().withStatus(Status.CONFLICT)
			...
			if (throwable instanceof HttpMessageConversionException) {
				return Problem.builder().withType(type).withTitle(status.getReasonPhrase()).withStatus(status)
						...
						.withCause(Optional.ofNullable(throwable.getCause())
								.filter(cause -> this.isCausalChainsEnabled()).map(this::toProblem).orElse(null));
			}
